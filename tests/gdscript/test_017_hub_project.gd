...
func test_017_add_dialog_accepts_blazium_and_godot() -> void:
	...
	assert_true(view.contains("run_project_async"), "run uses CLI")
	var panel := FileAccess.get_file_as_string("res://scenes/views/project_panel.tscn")
	assert_true(panel.contains("Run"), "run button")
