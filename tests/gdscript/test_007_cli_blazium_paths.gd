...
func test_007_packaging_shims_exist() -> void:
	...
	assert_true(iss.contains("--ensure-hub-remote"), "installer ensures hub_remote.json")
	assert_true(iss.contains("--hub-remote-path="), "installer can set machine hub_remote path")
	assert_true(iss.contains("runasoriginaluser"), "user ensure runs as original user")
